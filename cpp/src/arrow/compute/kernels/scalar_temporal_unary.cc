...
year_month_day GetFlooredYmd(int64_t arg, const int multiple,
  ...
  if (multiple == 1) {
    ...
    switch (options.unit) {
      ...
      default:
        return year_month_day(year{1970} / jan / 1);
    }
  } else {
...
const Duration FloorWeekTimePoint(const int64_t arg, const RoundTemporalOptions options,
  ...
  if (options.multiple == 1) {
    ...
    const auto w =
        weeks{floor<weeks>(t - start).count() / options.multiple * options.multiple} +
        start;
    return localizer_.template ConvertLocalToSys<Duration>(w.time_since_epoch(), st);
  } else {
